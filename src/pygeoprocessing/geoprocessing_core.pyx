...
def _calculate_pixel_intersection_stats(
        unset_geom, int xoff, int yoff,
        numpy.ndarray[numpy.int32_t, ndim=2] unset_fid_block,
        numpy.ndarray[numpy.float32_t, ndim=1] clipped_gt):
    cdef int block_x, block_y
    cdef float x_min, x_max, y_min, y_max
    cdef float gt_x, gt_y, gt_x_step, gt_y_step
    cdef float intersection_area
    gt_x = clipped_gt[0]
    gt_y = clipped_gt[3]
    gt_x_step = clipped_gt[1]
    gt_y_step = clipped_gt[5]
    aggregate_stats = {
    ...
    for block_y in range(unset_fid_block.shape[0]):
        for block_x in range(unset_fid_block.shape[1]):
            x_min = gt_x + gt_x_step * (block_x+xoff)
            y_min = gt_y + gt_y_step * (block_y+yoff)
            x_max = gt_x + gt_x_step * (block_x+xoff+1)
            y_max = gt_y + gt_y_step * (block_y+yoff+1)
            if x_max < x_min:
            ...
            if intersection_area > 0:
                ...
                if aggregate_stats['min'] is None:
                    aggregate_stats['min'] = val
                    aggregate_stats['max'] = val

                aggregate_stats['min'] = min(
                    val, aggregate_stats['min'])
                aggregate_stats['max'] = max(
                    val, aggregate_stats['max'])
                aggregate_stats['count'] += 1
                aggregate_stats['sum'] += val
